...
цветной шапке, ИТОГО сверху, колонка «Все бренды»). Лист «Маппинг позиций»
читается и дополняется новыми артикулами — ручные правки сохраняются.
...
Использование:
  python3 svod_report.py --keys api_keys.txt[,extra] --sa google_sa.json [--date YYYY-MM-DD] [--dry-run]
...
# ---------- данные из TrueStats ----------
wb_by_brand, oz_by_brand, union, errors = {}, {}, {}, []
for disp, prefix in BRANDS:
    contour = K.get(f"{prefix}_TS_TOKEN", "")
    token = K.get(f"TRUESTATS_TOKEN_{contour.upper()}", "")
    for mp, acc_key, dst in (("WB", f"{prefix}_TS_WB_ACCOUNTS", wb_by_brand),
                             ("OZON", f"{prefix}_TS_OZON_ACCOUNTS", oz_by_brand)):
        ...
        try:
            ...
            errors.append(f"{mp} {disp}: {str(e)[:200]}"); continue
        d = dst.setdefault(disp, {})
        ...
        for r in rows:
            ...
            if art and n:
                d[art] = d.get(art, 0) + n
                u[art] = 1
        print(f"{mp} {disp}: {len(rows)} строк")
        time.sleep(0.3)
...
for idx, ((name, ttl, color), agg) in enumerate(zip(SHEETS, (agg_all, agg_wb, agg_oz))):
    ...
    rows = [[f"{ttl} {DD}{warn}"] + [""] * (n_cols - 1),
            ["Итого"] + totals + [grand],
            ["Наименование"] + disp_names + ["Все бренды"]] + body_rows
    ...
    reqs += [
        ...
        {"updateDimensionProperties": {"range": {"sheetId": sid, "dimension": "COLUMNS",
            "startIndex": 1, "endIndex": n_cols}, "properties": {"pixelSize": 110}, "fields": "pixelSize"}},
